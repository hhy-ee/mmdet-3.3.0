...
class DDQVPDTransformerDecoder(DeformableDetrTransformerDecoder):
    ...
    def _init_layers(self) -> None:
        ...
        self.ref_point_head = MLP(self.embed_dims * 2, self.embed_dims,
                                  self.embed_dims, 2)
        self.dec_query_map = MLP(self.embed_dims + 4, self.embed_dims,
                                  self.embed_dims, 2)
    ...
    def forward(self, query: Tensor, value: Tensor, key_padding_mask: Tensor,
                ...
                valid_ratios: Tensor, reg_branches: nn.ModuleList, lstd_branches: nn.ModuleList,
                **kwargs) -> Tensor:
        ...
        intermediate_refpoint_stds = []
        self.cache_dict['distinct_query_mask'] = []
        train_with_vpd = self.cache_dict['train_with_vpd']
        ...
        for layer_index, layer in enumerate(self.layers):
            ...
            if not self.training:
                ...
                num_dn = self.cache_dict['dis_query_info'][0]
                ...
                new_refpoint_std = torch.zeros((query.size(0), query.size(1), 4), device=query.device)
                if train_with_vpd == 'all' or train_with_vpd == 'main':
                    tmp_topk = lstd_branches[layer_index](query[:, num_dn:-num_dense])
                    new_refpoint_std[:, num_dn:-num_dense, :] = tmp_topk.exp()
                if train_with_vpd == 'all' or train_with_vpd == 'aux':
                    tmp_dense = self.aux_lstd_branches[layer_index](query[:, -num_dense:])
                    new_refpoint_std[:, -num_dense:, :] = tmp_dense.exp()
                query = self.dec_query_map(torch.cat((query, new_refpoint_std), -1))
            ...
            if self.return_intermediate:
                ...
                intermediate_refpoint_stds.append(new_refpoint_std)
