...
from mmdet.structures.bbox import distance2bbox, bbox_cxcywh_to_xyxy, bbox_xyxy_to_cxcywh
...
class DDQDETRAuxVPDLoss(nn.Module):
    """DDQ auxiliary branches loss for dense queries.
    ...
    def __init__(
        ...
        super(DDQDETRAuxVPDLoss, self).__init__()
        self.train_cfg = train_cfg
        self.loss_cls = MODELS.build(loss_cls)
        self.loss_bbox = MODELS.build(loss_bbox)
        self.loss_dist_cfg = loss_dist
    ...
    def loss_single(self, cls_score, bbox_pred, bbox_dist, labels, label_weights,
                    bbox_targets, alignment_metrics, img_meta):
        """Calculate auxiliary branches loss for dense queries for one image.
    ...
    def loss_dist(self, pred, target, weight, avg_factor, img_meta):

        img_shape = img_meta['img_shape']
        tgt = target.clone()
        tgt[:, 0::2] = target[:, 0::2] / img_shape[1]
        tgt[:, 1::2] = target[:, 1::2] / img_shape[0]

        tgt = bbox_xyxy_to_cxcywh(tgt)
        tgt = inverse_sigmoid(tgt)
    ...
    def regularization_loss(self, dist, target, metric, project, scale_alpha, skew_beta, avg_factor):
        project = np.linspace(project[0], project[1], project[2])
        scale = (project.shape[0] - 1) / (project[-1] - project[0])
        acc = 1 / scale / 2
        target = (target.reshape(-1) - project[0]) * scale
    ...
    def loss(self, cls_scores, bbox_preds, bbox_dists, assign_with_vpd, gt_bboxes, gt_labels, img_metas,
             **kwargs):
        """Calculate auxiliary branches loss for dense queries.
        ...
        flatten_bbox_means = flatten_bbox_dists[..., :4].sigmoid()
        flatten_bbox_means = bbox_cxcywh_to_xyxy(flatten_bbox_means)
        bbox_means_list = []
        for img_id in range(len(bbox_preds)):
            det_bboxes = flatten_bbox_means[img_id]
            img_shape = img_metas[img_id]['img_shape']
            det_bboxes[:, 0::2] = det_bboxes[:, 0::2] * img_shape[1]
            det_bboxes[:, 1::2] = det_bboxes[:, 1::2] * img_shape[0]
            bbox_means_list.append(det_bboxes)
        flatten_bbox_means = torch.stack(bbox_means_list)
